...
import type { UserAnswerVO } from '@/Model/UserAnswer'
import { getUserAnswerVoService } from '@/api/userAnswer'
...
const loadData = async () => {
  ...
  const res = await getUserAnswerVoService(props.id)
  if (res.status === 200) {
    data.value = res.data
  } else {
    ...
    <a-card>
      <a-row style="margin-bottom: 16px">
        <a-col flex="auto" class="content-wrapper">
          ...
          <p>评分策略：{{ APP_SCORING_STRATEGY_MAP[data.scoreStrategy] }}</p>
          <p>
            <a-space>
              答题人：
