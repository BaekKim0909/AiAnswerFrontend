...
import type { QuestionContentDTO, QuestionVO } from '@/Model/Question'
import { Message } from '@arco-design/web-vue'
import { addQuestionService, editQuestionService, getQuestionVoService } from '@/api/question'
...
const loadData = async () => {
  ...
  const res = await getQuestionVoService(props.appId)
  if (res.status === 200 && res.data) {
    oldQuestion.value = res.data
...
const handleSubmit = async () => {
  ...
  if (oldQuestion.value?.id) {
    res = await editQuestionService({
      id: oldQuestion.value.id,
      questionContent: questionContent.value
    })
  } else {
    // 创建
    res = await addQuestionService({
