...
import { deleteScoringResultService, listScoringResultVoByPageService } from '@/api/scoringResult'
...
const loadData = async () => {
  ...
  const res = await listScoringResultVoByPageService(params)
  if (res.status === 200) {
...
const doDelete = async (record: ScoringResult) => {
  ...
  const res = await deleteScoringResultService(record.id)
  if (res.status === 200) {
    loadData()
  } else {
